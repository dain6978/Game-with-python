...
import pygame
pygame.init()
dyingSound = pygame.mixer.Sound('dying.wav')
dyingSound.set_volume(0.3)
shootingSound = pygame.mixer.Sound('shooting.wav')
shootingSound.set_volume(1)
bgmSound = pygame.mixer.Sound('bgm.wav')
shootingSound.set_volume(0.5)
...
class PlayerCannon(Actor):
    ...
    def __init__(self, x, y):
        ...
        self.isShoot = False # space를 pressed하고 있는 동안이 아닌 눌렀을 때 한 번만 실행되게 하기 위한 bool 변수
    ...
    def update(self, elapsed):
        ...
        # 미사일 발사 관련
        if (pressed[key.SPACE] == 1): # space 눌렀을 때
            if (self.isShoot == False):
                playerShoot = PlayerShoot(self.x, self.y + 50) # space 누를 때마다 미사일 객체 생성
                self.parent.add(playerShoot)
                self.parent.shoots.append(playerShoot)
                shootingSound.play()
                self.isShoot = True 
        else: # 0일 때 == release일 때
            self.isShoot = False

        movement = pressed[key.RIGHT] - pressed[key.LEFT]
        w = self.width * 0.5
        if movement != 0:
            if self.parent.width - w <= self.x: # 오른쪽 끝 (800 = gameLayer의 width)
                if (pressed[key.RIGHT] - pressed[key.LEFT] < 0): # 왼쪽 키를 누를 때만
                    self.move(self.speed * movement * elapsed) # move 가능, 그 외는 멈춤
            elif w >= self.x: # 왼쪽 끝
                if (pressed[key.LEFT] - pressed[key.RIGHT] < 0): # 오른쪽 키를 누를 때만
                    self.move(self.speed * movement * elapsed) # move 가능, 그 외는 멈춤
            else:
...
class AlienColumn(object):
    ...
    # def aliens_hit(): #alien 목숨 추가
    #     for (in in alien_types)
...
class PlayerShoot(Shoot):
    ...
    def collide(self, other):
        if isinstance(other, Alien):
            ...
            other.kill()
            self.kill() # alien과 충돌한 playerShoot kill
            
            
    def on_exit(self):
        super(PlayerShoot, self).on_exit()
        self.parent.shoots.remove(self) # layer 밖으로 나가거나 alien과 충돌한 shoot들은 shoots 리스트에서 제거
