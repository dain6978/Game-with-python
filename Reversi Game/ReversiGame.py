...
class HUD(cocos.layer.Layer):
    ...
    def update_score(self, person, computer): # score 업데이트
        self.score_text.element.text = 'You: %s, Computer: %s' % (person, computer)

    def show_game_over(self, winner): # 게임 오버
        w, h = cocos.director.director.get_window_size()
...
class GameLayer(cocos.layer.Layer): # 게임 Layer 클래스
    is_event_handler = True # 사용자 입력 받기 위한 이벤트 핸들러
    ...
    def update(self, dt):
        ...
        if computer+person == self.row*self.column or (len(moves1) == 0 and len(moves2) == 0): # 게임 승리 조건
            if computer > person:
                self.hud.show_game_over('Computer win') # 게임 오버 텍스트 호출
    ...
    def isPossible(self, x, y, turn, board): # 놓을 수 있는 disk에 대한 정보
        rtnList = list()
    ...
    def on_mouse_release(self, x, y, buttons, mod):
        ...
        if len(moves) > 0:    
            ...
            self.revList = self.isPossible(xx, yy, GameLayer.PERSON, self.table)

            if len(self.revList) == 0: return

            self.table[yy][xx] = GameLayer.PERSON
            for revX, revY in self.revList:
    ...
    def boardScore(self, board):
        ...
        if (self.difficulty == 0):
            self.easyScore(board)
        elif (self.difficulty == 1):
            self.normalScore(board)
        elif (self.difficulty == 2):
            self.hardScore(board)
    

    def easyScore(self, board): # disk 개수 차이
        computerWeightSum = 0
        personWeightSum = 0
        for y in range(0, self.row):
            for x in range(0, self.column):
    ...
    def normalScore(self, board): # weight 고려
        computerWeightSum = 0
        personWeightSum = 0
    ...
    def hardScore(self, board): # revList(뒤집을 수 있는 disk 수) 고려
        computerWeightSum = 0
        personWeightSum = 0
        ...
        for y in range(0, self.row):
            for x in range(0, self.column):
                if board[y][x] == GameLayer.COMPUTER:
                    computerWeightSum += self.weight[y][x]
                    computerWeightSum += len(self.revList)
                elif board[y][x] == GameLayer.PERSON:
                    personWeightSum += self.weight[y][x]
                    personWeightSum += len(self.revList)
